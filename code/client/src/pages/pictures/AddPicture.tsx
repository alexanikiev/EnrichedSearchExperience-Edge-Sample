...
class AddPicture extends Component<IAddPictureProps, IAddPictureState> {
    
    constructor(props: IAddPictureProps) {
        super(props);
    ...
    filesChange(event: any) {
        ...
        for (var i = 0; i < event.target.files.length; i++) {
           ...
           var file = { picture: event.target.files[i],
                        name: filename, 
    ...
    formSubmit(event: any) {
        ...
        let mostRecentPromise = this.state.files.reduce((previousPromise, file) => {
            return previousPromise.then((filesSoFar: any) => {
                ...
                data.append('pictureFile', file.picture);
                data.append('pictureFileName', file.name);

                return fetch('http://localhost:9991/cracking/pictures', {
                    method: 'post',
                    body: data
                })
                .then(result => {
