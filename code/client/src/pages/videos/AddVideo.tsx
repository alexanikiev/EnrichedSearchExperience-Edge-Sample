...
class AddVideo extends Component<IAddVideoProps, IAddVideoState> {
    
    constructor(props: IAddVideoProps) {
        super(props);
    ...
    filesChange(event: any) {
        ...
        for (var i = 0; i < event.target.files.length; i++) {
           ...
           var file = { video: event.target.files[i],
                        name: filename, 
    ...
    formSubmit(event: any) {
        ...
        let mostRecentPromise = this.state.files.reduce((previousPromise, file) => {
            return previousPromise.then((filesSoFar: any) => {
                ...
                data.append('videoFile', file.video);
                data.append('videoFileName', file.name);

                return fetch('http://localhost:9991/cracking/videos', {
                    method: 'post',
                    body: data
                })
                .then(result => {
